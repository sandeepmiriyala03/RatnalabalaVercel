...
/* ─────────────────────────────────────────────────────────────────
   COLORS — every value below is a CSS variable defined once in
   globals.css, not a local hex palette. This page used to carry its
   own separate hardcoded colors (a different maroon, a different
   teal) which drifted from the navbar and the rest of the site.
   Reading from the shared tokens means this page is now guaranteed
   to match everywhere else, including automatically adapting in
   dark mode since those variables already flip there.
   ───────────────────────────────────────────────────────────────── */
const INK = "var(--foreground)";
const PAPER = "var(--background)";
const GOLD = "var(--accent)";           // large/decorative use only (icons, fills) — see globals.css audit
  ...
  return (
    <Box
      sx={{
        ...
        bgcolor: PAPER, border: `2px solid ${color}`,
...
export default function RatnalabalaHighlights() {
  ...
  return (
    <Box sx={{ bgcolor: PAPER }}>
      {/* ═══════════ HERO — forest-green band (matches the navbar bar
          exactly, same --secondary token), ivory text, gold hairline
          and citation. Sample verse lives here as an epigraph instead
          of a separate card further down. ═══════════ */}
      <Box sx={{ bgcolor: TEAL, color: PAPER, py: { xs: 6, md: 8 } }}>
        <Container maxWidth="md">
          <Reveal delay={0}>
            ...
            <Typography
              fontFamily="'Noto Serif Telugu', serif"
              textAlign="center"
              sx={{ fontSize: { xs: "0.98rem", md: "1.05rem" }, lineHeight: 2, whiteSpace: "pre-line", opacity: 0.92 }}
            >
...
సమరసభావ అఃకలిత వర్ణనుతం బసవేశ పాహిమాం!!`}
      ...
      <Container maxWidth="md" sx={{ py: { xs: 4, md: 5 } }}>
        {/* ═══════════ ABOUT — short lede; full paragraph one tap away. ═══ */}
        <Reveal delay={0.06}>
          <Box sx={{ mb: 4 }}>
            <Typography
            ...
            <Box textAlign="center" sx={{ mt: 1 }}>
              <Button
                ...
                size="small"
                sx={{ textTransform: "none", color: MAROON, fontWeight: 700 }}
            ...
            <Collapse in={aboutOpen} timeout={240} unmountOnExit>
              <Card sx={{ mt: 2, borderRadius: "var(--radius)", border: `1.5px solid ${BORDER_STRONG}`, bgcolor: CARD_BG }}>
                <CardContent>
                  <Typography sx={{ fontSize: "0.95rem", color: INK, lineHeight: 1.9, textAlign: "justify" }}>
        ...
        {loading && (
          <Stack alignItems="center" spacing={1.5} sx={{ py: 4 }}>
            ...
            <Typography sx={{ fontSize: "0.9rem", color: MUTED }}>పద్యాలు లోడ్ అవుతున్నాయి…</Typography>
          </Stack>
        )}

        {error && (
          <Typography align="center" sx={{ py: 3, fontSize: "0.95rem", color: MAROON }}>
            {error}
          </Typography>
        )}
        ...
        {!loading && !error && poems.length > 0 && (
          <Reveal delay={0.12}>
            <Card sx={{ mb: 4, borderRadius: "var(--radius)", border: `1.5px solid ${BORDER_STRONG}`, bgcolor: CARD_BG }}>
        ...
        {/* ═══════════ MODULE MALA — gold thread with a bead per group,
            each group colored from the same three brand tokens
            (primary / secondary / accent) used across the whole site. ═══ */}
        <Reveal delay={0.18}>
          <Typography
            fontFamily="'Noto Serif Telugu', serif"
            textAlign="center"
          ...
          <Box sx={{ position: "relative", pl: { xs: 3, sm: 4 } }}>
            <Box
              ...
              sx={{
                ...
                width: 2, bgcolor: "color-mix(in srgb, var(--accent) 45%, transparent)",
              }}
            />
                    ...
                    {group.items.map((item) => (
                      <Box
                        ...
                        sx={{
                          ...
                          borderRadius: "var(--radius-sm)", p: 1.5, bgcolor: CARD_BG, border: "1px solid",
                          borderColor: `color-mix(in srgb, ${group.color} 25%, transparent)`,
                          transition: "transform 0.15s ease, box-shadow 0.15s ease",
                          "&:hover": {
                            transform: "translateY(-2px)",
                            boxShadow: `0 6px 16px color-mix(in srgb, ${group.color} 16%, transparent)`,
                          },
                          "&:focus-visible": { outline: `3px solid ${group.color}`, outlineOffset: "2px" },
                          ...reducedMotionOff,
                        }}
                      >
        ...
        <Reveal delay={0.24}>
          <Box
            sx={{
              mt: 5, mb: 4, p: { xs: 2.5, md: 3 }, borderRadius: "var(--radius)", textAlign: "center",
              bgcolor: TEAL, border: `1px solid ${GOLD}`,
            }}
          >
            <Stack direction="row" spacing={1} alignItems="center" justifyContent="center" sx={{ mb: 0.5 }}>
              ...
              <Typography sx={{ fontWeight: 800, fontSize: { xs: "1.05rem", md: "1.2rem" }, color: PAPER }}>
                ఈరోజు నిర్ణయాత్మక పద్యం
              </Typography>
            </Stack>
            <Typography sx={{ color: PAPER, opacity: 0.85, fontSize: "0.9rem", mb: 2 }}>
              1140+ పద్యాలలో నుండి — ఒక్క క్లిక్‌తో మీ కోసం ఎంపిక
            </Typography>
            <Box sx={{ bgcolor: PAPER, borderRadius: "var(--radius)", p: { xs: 1.5, md: 2 } }}>
              <AgentPoemButton />
            </Box>
          </Box>
        </Reveal>
        ...
        <Reveal delay={0.3}>
          <Box sx={{ mb: 2 }}>
            <Button
              ...
              sx={{ textTransform: "none", color: MUTED, fontWeight: 700 }}
            >
              స్వరాల గురించి
            </Button>

            <Collapse in={voicesOpen} timeout={240} unmountOnExit>
              <Card sx={{ mt: 1.5, borderRadius: "var(--radius)", bgcolor: CARD_BG }}>
                <CardContent>
                  <Typography sx={{ fontSize: "0.88rem", color: INK, lineHeight: 1.7, mb: 1 }}>
